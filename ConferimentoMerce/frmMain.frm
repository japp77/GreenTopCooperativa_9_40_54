...
Dim IDTesta As Long
    ...
    If fnMsgQuestion(gResource.GetCustomizedMessage(MESS_QUERYREMOVE), m_App.FunctionName) = vbYes Then
        ...
        IDTesta = fnNotNullN(m_Document(m_Document.PrimaryKey).Value)
        ...
        'ELIMINA RIGHE DOCUMENTO
        sSQL = "DELETE FROM RV_POCaricoMerceRighe "
        sSQL = sSQL & "WHERE IDRV_POCaricoMerceTesta=" & IDTesta
        Cn.Execute sSQL
        
        'ELIMINA IMBALLI
        sSQL = "DELETE FROM RV_POCaricoMerceImballi "
        sSQL = sSQL & "WHERE IDRV_POCaricoMerceTesta=" & IDTesta
        Cn.Execute sSQL
...
Private Sub SendDocument(ByVal Appl As Long)
    ...
    Screen.MousePointer = vbHourglass
    'm_Report.SendMailTo ecPdf, "marco.berini@pointoffice.it"
